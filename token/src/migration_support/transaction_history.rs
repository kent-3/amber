...
use super::append_store::{AppendStore, AppendStoreMut};

use super::old_state::Config;
...
fn append_transfer<S: Storage>(
    ...
    let mut store = PrefixedStorage::multilevel(store, &[PREFIX_TRANSFERS, for_address.as_slice()]);
    let mut store = AppendStoreMut::attach_or_create(&mut store)?;
    store.push(tx)
}
